...
To re-import a page:
1. Edit the parser, transformer, or `tools/importer/import.js` (single universal script for all templates)
...
The universal import script auto-detects the correct template by matching the incoming URL against each template's `urls` array. All 14 parsers are registered, all 10 templates are embedded.
